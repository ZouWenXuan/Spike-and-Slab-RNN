...
#             gpi_in += -1/(self.tau_m)*self.m_in*np.dot(z[tt], x[tt].T)+\
#                   -1/(2*self.tau_m)*((2*self.pi_in-1)*self.m_in**2+self.xi_in)\
#                   *np.dot(z[tt]*epsi1[tt]/delta1[tt]*self.variance_fix(delta[tt]), (x[tt]**2).T)
#             gxi_in += 1/(2*self.tau_m)*(1-self.pi_in)\
#                     *np.dot(z[tt]*epsi1[tt]/delta1[tt]*self.variance_fix(delta[tt]), (x[tt]**2).T)
    ...
    #梯度裁剪
        ...
        gxi_in, gxi_rec, gxi_out = 0, grad_clip(gxi_rec/b_size,10), grad_clip(gxi_out/b_size,10)
        gpi_in, gpi_rec, gpi_out =0, grad_clip(gpi_rec/b_size,10), grad_clip(gpi_out/b_size,10)
...
def train(net, b_size, batches, train_data, train_label, learn_rate, optimizer):
    ...
    for ii in range(batches):
        x = (data[ii*b_size:(ii+1)*b_size]).T.reshape(784,1,b_size)
...
def test(net, test_data, test_label,sampling=False):  
    ...
    for ii in range(batches):      
        x_test = (data[ii*b_size:(ii+1)*b_size].T).reshape(784,1,b_size)      
...
for epoch in range(Total_epoch):
    ...
    np.save('RNN-SAS-pixel/3/RNN-SAS-m_in_all',m_in_all, allow_pickle=True)
    np.save('RNN-SAS-pixel/3/RNN-SAS-pi_in_all',pi_in_all, allow_pickle=True)
    np.save('RNN-SAS-pixel/3/RNN-SAS-sigma_in_all',sig_in_all, allow_pickle=True)
    
    np.save('RNN-SAS-pixel/2/RNN-SAS-m_out_all',m_out_all, allow_pickle=True)
    np.save('RNN-SAS-pixel/2/RNN-SAS-pi_out_all',pi_out_all, allow_pickle=True)
    np.save('RNN-SAS-pixel/2/RNN-SAS-sigma_out_all',sig_out_all, allow_pickle=True)
            
    np.save('RNN-SAS-pixel/2/RNN-SAS-m_rec_all',m_rec_all, allow_pickle=True)
    np.save('RNN-SAS-pixel/2/RNN-SAS-pi_rec_all',pi_rec_all, allow_pickle=True)
    np.save('RNN-SAS-pixel/2/RNN-SAS-sigma_rec_all',sig_rec_all, allow_pickle=True)
    ...
    np.save('RNN-SAS-pixel/2/accuracy',accuracies, allow_pickle=True)    
end = time.time()
